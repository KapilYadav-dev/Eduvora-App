package in.kay.edvora.Views.Fragments;

import androidx.lifecycle.ViewModelProvider;

import android.os.Bundle;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import in.kay.edvora.R;
import in.kay.edvora.ViewModel.AskQuestionViewModel;

public class AskQuestion extends Fragment {

    private AskQuestionViewModel mViewModel;

    public static AskQuestion newInstance() {
        return new AskQuestion();
    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.ask_question_fragment, container, false);
    }

    @Override
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        super.onActivityCreated(savedInstanceState);
        mViewModel = new ViewModelProvider(this).get(AskQuestionViewModel.class);
        // TODO: Use the ViewModel
    }

}